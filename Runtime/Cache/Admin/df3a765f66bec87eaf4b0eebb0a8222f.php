...
<?php if(is_array($info)): foreach($info as $key=>$t): $access = IndexPublic::GS('User_group_table',$gid); ?>
 <?php if($t['type']=='='){ $view = unserialize($t['view']); if(strstr($t['level'],$access) || ($group_access>=999 && $t['level']<9999) || in_array($uid,$view)){ ?>
  <div title="<?php echo ($t["text"]); ?>" data-options="iconCls:'<?php echo ($t["iconCls"]); ?>'">
   <ul class="easyui-tree left-tree" data-options="url:'__GROUP__/Index/json/mid/<?php echo ($t["id"]); ?>',editable:false,lines:true,method:'get',onClick:function(node){onClickTree(node);}"></ul>
  </div>
  <?php
 } ?>
 <?php }else{ ?>
...
-->
 ...
 <div data-options="region:'center',split:true">
  <div id="rightTabs" class="easyui-tabs" data-options="fit:true,border:false">
   <?php if($type==1){ ?><div title="最新跟进" data-options="closable:false,id:-3,href:'__URL__/myview'"></div>   
   <?php }else{ ?>
    <div title="控制台" data-options="closable:false,id:-1,href:'__URL__/main'"></div><?php } ?>
  </div>
 </div>
 <div id="repwd"></div>
 <div id="setpwd"></div>
