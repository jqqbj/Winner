...
$(function(){
	...
	$("#companyId<?php echo ($uniqid); ?>").combobox({
		url:'__ITEM__/__RUNTIME__/Data/Json/Comy_data.json',
		editable:false,
		method:'get',
		required:true,
		valueField:'id',
...
<form id="addFormUser<?php echo ($uniqid); ?>" method="post">
 <table class="infobox" width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <?php if(C('MORE_COMY')){ ?><tr>
    ...
    <td width="77%">
    <input name="company_id" class="relo" id="companyId<?php echo ($uniqid); ?>" size="28" />
    </td>
  </tr>
  <tr>
    <td width="23%"><label for="part_id">部门</label></td>
    <td width="77%">
    <input name="part_id" class="easyui-combobox relo" id="partId<?php echo ($uniqid); ?>" size="28" data-options="url:'__ITEM__/__RUNTIME__/Data/Json/Empty_data.json',editable:false,method:'get',required:false,valueField:'id',textField:'text'" />
    </td>
  </tr>
  <?php }else{ ?>
  <tr>
    <td width="23%"><label for="part_id">部门</label></td>
    <td width="77%">
    <input name="part_id" class="easyui-combobox relo" id="partId<?php echo ($uniqid); ?>" size="28" data-options="url:'__ITEM__/__RUNTIME__/Data/Json/Part_data.json',editable:false,method:'get',required:true,valueField:'id',textField:'text'" />
    </td>
  </tr><?php } ?>
  <tr>
    <td width="23%"><label for="group_id">角色</label></td>
    <td width="77%">
   <input name="group_id" class="easyui-combobox relo" id="group_ids<?php echo ($uniqid); ?>" size="28" data-options="url:'__ITEM__/__RUNTIME__/Data/Json/Group_data.json',editable:false,method:'get',required:true,valueField:'id',textField:'text'" />
    </td>
  </tr>
  <tr>
    <td width="23%"><label for="password">密码</label></td>
    <td width="77%"> <input name="password" type="password" class="easyui-validatebox"  size="22" /><?php if($act=='add'){ ?>(留空时自动生成)<?php } ?></td>
  </tr>
  <tr>
